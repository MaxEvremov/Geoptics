...
module.exports.checkSensorAccess = (req, res, next) => {
    ...
    if(is_get && req.query.sensor_id || req.body.sensor_id) {
        sensor_ids = [is_get
            ? req.query.sensor_id
            : req.body.sensor_id
        ]
    }
    else if(is_get && req.query.sensor_ids || req.body.sensor_ids) {
        sensor_ids = is_get
            ? req.query.sensor_ids
            : req.body.sensor_ids
    }
    ...
    async.parallel(
        ...
        (err, result) => {
            ...
            let available_well_ids = result.wells.map((w) => w.id)
            let sensor_well_ids = result.sensor.map((s) => s.well_id)
