...
// override default node-pg date parser
...
let default_date_parser = types.getTypeParser(TIMESTAMP_OID)

let date_parser = (date) => helpers.convertDate(default_date_parser(date), "native", "iso8601")
...
helpers.validateRequestData = (fields) => (req, res, next) => {
...
helpers.validatePermissions = (roles) => (req, res, next) => {
    if(!_.isArray(roles)) {
        roles = [roles]
    }
...
helpers.checkWellAccess = (req, res, next) => {
...
helpers.checkSensorAccess = (req, res, next) => {
    let is_get = (req.method === "GET")
    ...
    async.parallel(
        {
            ...
            sensors: (done) => helpers.makePGQuery(sensors_query, done)
        },
        (err, result) => {
            if(err) {
                return res.jsonCallback("no_access")
...
helpers.makePGQuery = (query, options, done) => {
...
helpers.convertDate = (date, from, to) => {
    var from_moment
    ...
    if(to === "iso8601" && from_moment) {
        //console.log(from_moment.format("YYYY-MM-DD HH:mm:ss.SSSSSSZ"))
...
helpers.logRequests = (req, res, next) => {
    if(!req.user) {
        return next()
    }
